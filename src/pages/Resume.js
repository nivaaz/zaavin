...
class Resume extends React.Component {
  ...
  handleTagClick = e => {
    e.preventDefault()
    console.log("HANDLE TAG CLICK", e.target.value, e.target.id)
  ...
  storyModeClick = e => {
    ...
    this.setState(
      ...
      console.log("storymode", this.state.storyMode)
    )
  }
  renderTags = (tech, id) => {
    let isSelected = "";
    console.log("renderTags", tech, id)
    return tech.map((key, ind) => {
      if (id === "Technical Skills"){
         isSelected = this.state.TechnicalTags[ind] === true
        ? styles.selected
        : styles.unselected
      } else{
         isSelected = this.state.SoftTags[ind] === true
        ? styles.selected
        : styles.unselected
      }
      return (
  ...
  renderResumeTag = () => {
    ...
    return (
      <div className={styles.selectStory}>
        ...
        {this.renderTags(tech, "Technical Skills")}
        <h3> Soft Skills </h3>
        {this.renderTags(soft, "Soft Skills")}
      </div>
    )
  }
